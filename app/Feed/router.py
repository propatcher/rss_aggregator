...
async def delete_rss(feed_id:int,current_user: User = Depends(get_current_user)):
...
@router.put("/edit")
async def update_rss(Feed:SFeed,url:HttpUrl,current_user: User = Depends(get_current_user)):
    new_feed = await FeedDAO.update_feed(current_user.id,url,Feed.title,Feed.url)
    return SFeedResponse(
        id = new_feed.id,
        url= new_feed.url,
        title= new_feed.title,
